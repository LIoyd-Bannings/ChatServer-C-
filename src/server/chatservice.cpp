...
// 处理登陆业务 ORM 业务层操作的都是对象 数据层才是数据操作   id  pwd
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPwd() == pwd)
    {
        ...
        else
        {
            ...
            // 查询该用户所在组的消息并返回
            vector<Group> GroupVecs = _groupModel.queryGroups(id);
            if (!GroupVecs.empty())
            {
                vector<string> vec3;
                for (Group &group : GroupVecs)
                {
                    json js;
                    js["id"] = group.getId();
                    ...
                    for (GroupUser &user : group.getUsers())
                    {
                        ...
                        userStrs.push_back(userJs.dump());
                    }
                    js["users"] = userStrs;
